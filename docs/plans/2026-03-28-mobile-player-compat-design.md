...
- app.js
  ...
  - `generate()` 中仅预连稳定音频流，不展示播放器
  - `appendAudioChunk()` 中移动端稳定流模式不再触发播放器显示
  - `onGenerateDone()` 中按链路类型统一展示播放器并自动起播
...
- iOS Safari：生成中不显示底部播放器，done 后显示并自动起播
- iOS Chrome：同上
...
- HTTP 流模式下，done 前播放器虽然不展示，但底层连接已建立，需要保证断流时正确清理
